...
/*
 * Https.class
 * Https request, responseë¥? ê´?ë¦¬í•˜?Š” class ?…?‹ˆ?‹¤.
 */
public class Https
{
	/*
	...
	public JSONObject postRequest(String url_string, HashMap<String, String> params) {
		...
		try {
			...
			// data ?ƒ?„± ë°? ?°?´?„° êµ¬ë¶„?„ ?œ„?•œ delimiter ?„¤? •
			StringBuffer postDataBuilder = new StringBuffer();
			postDataBuilder.append(delimiter);

			// params?— imageê°? ?ˆ?œ¼ë©? ë³??ˆ˜?— ?‹´?•„ requestë¥? ?‹¤ë¥´ê²Œ ë³´ë‚¸?‹¤
			String image = null;
	...
	public JSONObject request(String url_string, HashMap<String, String> params) {
		...
		try {
			...
			String response = null;
			String inputLine; // ?„œë²„ë¡œ ë¶??„° ë°›ì? responseë¥? ë°›ì„ ë³??ˆ˜
			while ((inputLine = in.readLine()) != null) {
				response = inputLine;
			}

			if (response != null) {
				// response ê°? object ?ƒ array?— ?”°?¼ parseë¥? ?‹¤ë¥´ê²Œ?•œ?‹¤.
				try {
					obj = (JSONObject) JSONValue.parse(response);
				} catch (Exception e) {
					try {
	...
	/*
     * ?—…ë¡œë“œ?•  ?ŒŒ?¼?— ???•œ ë©”í? ?°?´?„°ë¥? ?„¤? •?•œ?‹¤.
     * @param key : ?„œë²„ì—?„œ ?‚¬?š©?•  ?ŒŒ?¼ ë³??ˆ˜ëª?
     * @param fileName : ?„œë²„ì—?„œ ???¥?  ?ŒŒ?¼ëª?
     */
	...
	/*
     * Map ?˜•?‹?œ¼ë¡? Key?? Valueë¥? ?…‹?Œ…?•œ?‹¤.
     * @param key : ?„œë²„ì—?„œ ?‚¬?š©?•  ë³??ˆ˜ëª?
     * @param value : ë³??ˆ˜ëª…ì— ?•´?‹¹?•˜?Š” ?‹¤? œ ê°?
     */
